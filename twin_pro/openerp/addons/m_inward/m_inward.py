...
class m_inward(osv.osv):
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'm.inward', context=c),
		'active': True,
		'state': 'draft',
	...
	def _validations(self, cr, uid,ids, context=None):
		...
		if rec.name:
			cr.execute(""" select upper(name) from m_inward where upper(name) = '%s' """ %(rec.name.upper()))
			data = cr.dictfetchall()
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Inward Name already exists'))
		if rec.code:
			cr.execute(""" select upper(code) from m_inward where upper(code) = '%s' """ %(rec.code.upper()))
			data = cr.dictfetchall()
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Inward Code already exists'))
		return True
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(m_inward, self).write(cr, uid, ids, vals, context)
	
	_constraints = [
		
		(_validations, ' Validations of Inward master ', ['']),
