...
class stock_location(osv.osv):
	...
	_columns = {
		...
		'location_id': fields.many2one('stock.location', 'Parent Location', select=True, ondelete='cascade',domain="[('is_parent','=',True),('state','=','approved')]"),
	...
	def _validations(self, cr, uid,ids, context=None):
		...
		if rec.name:
			...
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Location already exists'))
		if rec.code:
			cr.execute(""" select upper(code) from stock_location where upper(code) = '%s' """ %(rec.code.upper()))
			data = cr.dictfetchall()
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Location Code already exists'))
		return True
	
	_constraints = [
		
		(_validations, ' Validations of Location master ', ['']),
