...
class m_payment_terms(osv.osv):
	
	_name = "m.payment.terms"
	_description = "Payment Terms Master"
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'm.payment.terms', context=c),
		'active': True,
		'state': 'draft',
	...
	def _validations(self, cr, uid,ids, context=None):
		...
		if rec.name:
			cr.execute(""" select upper(name) from m_payment_terms where upper(name) = '%s' """ %(rec.name.upper()))
			data = cr.dictfetchall()
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Payment Term Name already exists'))
		if rec.code:
			cr.execute(""" select upper(code) from m_payment_terms where upper(code) = '%s' """ %(rec.code.upper()))
			data = cr.dictfetchall()
			if len(data) > 1:
				raise osv.except_osv(_('Warning !'),_('Payment Term Code already exists'))
		return True
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(m_payment_terms, self).write(cr, uid, ids, vals, context)
	
	_constraints = [
		
		(_validations, ' Validations of Payment Terms master ', ['']),
