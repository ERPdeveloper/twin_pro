...
class t_sale_order(osv.osv):
	...
	def _get_order(self, cr, uid, ids, context=None):
		result = {}
		for line in self.pool.get('ch.sale.order').browse(cr, uid, ids, context=context):
	...
	_columns = {
		...
		'so_date': fields.date('SO Date',required=True),
		...
		'amount_untaxed': fields.function(_amount_all, digits_compute= dp.get_precision('Account'), string='Untaxed Amount',
			store={
				'ch.sale.order': (_get_order, None, 10),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 't.sale.order', context=c),			
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(t_sale_order, self).write(cr, uid, ids, vals, context)
